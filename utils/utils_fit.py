...
def fit_one_epoch(model_train, model, ssd_loss, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, cuda, fp16, scaler, save_period, save_dir, local_rank=0):
    ...
    if local_rank == 0:
        ...
        if len(loss_history.val_loss) <= 1 or (val_loss / epoch_step_val) <= min(loss_history.val_loss):
            print('Save best model to best_epoch_weights_vgg.pth')
            torch.save(model.state_dict(), os.path.join(save_dir, "best_epoch_weights_vgg.pth"))
            
        torch.save(model.state_dict(), os.path.join(save_dir, "last_epoch_weights_vgg.pth"))